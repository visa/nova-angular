...
/** #custom **/
@Component({
  selector: 'nova-workshop-accordion-with-arrow-keys-and-no-tab-navigation',
  templateUrl: './with-arrow-keys-and-no-tab-navigation.docs.html',
  standalone: true,
  imports: [CommonModule, NovaLibModule, VisaChevronRightTiny, VisaChevronDownTiny]
})
export class ArrowKeysAndNoTabNavAccordionComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    if (this.buttons.length > 0) {
      this.novaLibService.addArrowKeyNavigation(this.buttons.toArray(), true);
    }
  }
}

...
@Component({
  selector: 'nova-workshop-set-of-chips-delete-chips',
  templateUrl: './set-of-chips-delete-chips.docs.html',
  standalone: true,
  imports: [CommonModule, NovaLibModule, VisaClearAltTiny]
})
export class SetOfChipsDeleteChipsChipComponent {
  initiallabels = ['Label 1', 'Label 2', 'Label 3', 'Label 4', 'Label 5', 'Label 6', 'Label 7'];
  labels = this.initiallabels;

  makeButtonId(label: string) {
    return 'removable-' + label.replace(/\s/g, '-').toLowerCase();
  }
    ...
    for (let i = 0; i < this.labels.length; i++) {
      if (this.labels[i] === labelToDelete) {
        if (i === 0 && this.labels.length === 1) {
          ...
          labelToFocus = 'removable-reset-button';

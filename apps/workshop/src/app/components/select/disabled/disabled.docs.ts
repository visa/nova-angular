...
/** #docs **/
@Component({
  selector: 'nova-workshop-select-disabled',
...
export class DisabledSelectComponent {
  ...
  disableSelect(event: Event) {
    ...
    const isChecked: undefined | null | boolean = target?.checked;

    if (target && (isChecked !== null || isChecked !== undefined)) {
      this.isSelectDisabled = isChecked;
    }
  }
}

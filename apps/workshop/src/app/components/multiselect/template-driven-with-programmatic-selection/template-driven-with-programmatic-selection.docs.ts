...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, NovaLibModule, VisaErrorTiny, VisaChevronDownTiny, VisaChevronUpTiny]
})
export class TemplateDrivenProgrammaticSelectionMultiselectComponent {
  ...
  val: { label: string; value: string } | null = this.filteredItems[0];
  ...
  selectItem(item: { label: string; value: string } | null) {
    this.val = item;
  }
}

...
import { AccordionDirective, InputDirective, NovaLibModule } from '@visa/nova-angular';
import {
...
@Component({
  selector: 'nova-workshop-wizard-in-page-custom-markup',
  templateUrl: './in-page-custom-markup.docs.html',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class InPageCustomMarkupWizardComponent {
  ...
  goTo(index: number) {
    ...
    if (this.steps[index]) {
      ...
      setTimeout(() => {
        ...
        this.accordion.headings.toArray()[index].hostButton?.el.nativeElement.focus();
      }, 0);
    }
  }
